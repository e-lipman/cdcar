...
#'
repair.JP.county_ERL <- function(dat.I.JP = list(), dat.D.JP = list(), dat.I = list(), dat.D = list(), h = 7, date.start = as.Date("2020-03-01"),
                                 show_plots = TRUE){
  ...
  if (nrow(ind.I)>1){ ind.I = ind.I[order(ind.I[, 1]), ] }
  n.I = nrow(ind.I)
  ...
  if (nrow(ind.D) >1) {ind.D = ind.D[order(ind.D[, 1]),]}
  n.D = nrow(ind.D)
